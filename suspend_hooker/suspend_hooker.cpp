...
struct TargetAddress {
    ...
    u_int64_t hook1_offset() {return target_base + 0x11b7;}
    u_int64_t cave1_offset() {return target_base + 0x1200;}
    u_int64_t hook1_end_offset() {return target_base + 0x11bd;}
};
    ...
    if (pid == 0) { // Target
        ...
        if (WIFSTOPPED(status)) {
            ...
            if (WIFSTOPPED(status) && WSTOPSIG(status) == SIGTRAP) {
                ...
                printf("jmp bridge1\n");
                int override_length = 6;
                read_bytes = read_data(pid, target_address.hook1_offset(), override_length);
                
                write_data(pid, target_address.hook1_offset(), 
                            get_rel_jmp(target_address.hook1_offset(), target_address.cave1_offset()));
                write_data(pid, target_address.cave1_offset(), 
                            get_abs_jmp(inject_lib_offset.bridge1_offset()));
                
                printf("在 bridge1 結尾執行原本指令並跳回 main\n");
                
                write_data(pid, inject_lib_offset.bridge1_trampoline(), 
                            read_bytes);
                write_data(pid, inject_lib_offset.bridge1_trampoline() + override_length, 
                            get_abs_jmp(target_address.hook1_end_offset()));
